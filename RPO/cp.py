...
for DATASET in DATASETS:
    for SEED in [1,2,3]:
        base_path = f'output/rpo/base2new/train_base/{DATASET}/shots_{SHOTS}/{TRAINER}/{CFG}/seed{SEED}'
        new_path = f"res/{DATASET}/{TRAINER}/base/seed{SEED}"
        os.makedirs(new_path, exist_ok=True)
        shutil.copyfile(base_path+'/log.txt',new_path+'/log.txt')

        for SUB in SUBS:
            origin_path = f'output/rpo/base2new/test_{SUB}/{DATASET}/shots_{SHOTS}/{TRAINER}/{CFG}/seed{SEED}'
